...
export function parseGedcom(gedcomText: string): Partial<OrbData> {
  ...
  for (const rawLine of lines) {
    ...
    if (current.type === 'INDI') {
      ...
      if (level === 1 && tag === 'NAME') {
        indi.name = value.replace(/\//g, '').replace(/\s+/g, ' ').trim()
      } else if (level === 1 && tag === 'SEX') {
        indi.sex = value
      } else if (level === 1 && (tag === 'BIRT' || tag === 'DEAT')) {
        currentEvent = tag as 'BIRT' | 'DEAT'
        ...
        indi.famc = [...(indi.famc || []), cleanId(value)]
      } else if (level === 1 && tag === 'FAMS') {
        indi.fams = [...(indi.fams || []), cleanId(value)]
      }
...
export function extractBondsFromGedcom(
  gedcomText: string,
  persons: { id: string }[]
): Bond[] {
  const lines = gedcomText
  ...
  for (const raw of lines) {
    ...
    if (level === 0) {
      flushAsso()
      currentIndiId =
        tag === 'INDI' && pointer ? pointer.replace(/@/g, '') : null
      continue
    }
